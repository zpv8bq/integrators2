...
for d in dimensions:
    ...
    for N in Ns:
        ...
        stdev = float([line for line in output if "stat uncertainty" in line][0].split(":")[1])          #Uncertanties did not come out correctly, could not figure out why
        errors[d].append(relerror)
        uncertainties[d].append(stdev/math.sqrt(N))
...
d=10
sqrtN = np.sqrt(Ns)
plt.plot(sqrtN, errors[d],'o-', label=f"d={d}")   
plt.xscale("log")
plt.yscale("log")
plt.xlabel(r"$\sqrt{N}$")
plt.ylabel("Fractional Error")
plt.title("Fractional Error for d = 10")
plt.legend()
plt.grid(True, which="both", ls="--", lw=0.5)
plt.tight_layout()
plt.savefig("FractionalError.png", dpi=300)
plt.show()
...
for d in dimensions:
    sqrtN = np.sqrt(Ns)
    plt.plot(sqrtN, errors[d],'o-', label=f"d={d}")   
	#left out error bars, could not figure out
